---
title: Environment Variables
description: Environment variables in the Novadesk JavaScript API
---

import CodeBlock from '@theme/CodeBlock';
import {Method, MethodParam} from '@site/src/components/Method';
import Admonition from '@theme/Admonition';

<Admonition type="warning">
The `system` object is **only available in the Main script**. UI scripts should communicate with the main script via [IPC](/api/widget-api/widget-methods/#inter-process-communication-ipc) if they need system data.
</Admonition>

## Methods

<Method signature="system.getEnv(name)"
  description="Get the value of an environment variable.">
  ### Parameters
  <MethodParam name="name" type="string">
    Name of the environment variable. If omitted, all environment variables are returned.
  </MethodParam>
    <Admonition type="tip">
Omitting the parameter will return all environment variables.
<CodeBlock language="js">
  {`var allEnv = system.getEnv();`}
</CodeBlock>
</Admonition>
  ---
  <MethodParam name="Return Value" type="string | Object | null">
    Returns the value of the environment variable, `null` if it does not exist, or an object containing all environment variables when no name is provided.
  </MethodParam>
</Method>

## Examples

### Getting a Specific Environment Variable

<CodeBlock language="javascript">
  {`// Get the PATH environment variable
var path = system.getEnv("PATH");
console.log("Current PATH: " + path);

// Get the username
var username = system.getEnv("USERNAME");
console.log("Current user: " + username);

// Get the TEMP directory
var tempDir = system.getEnv("TEMP");
console.log("Temp directory: " + tempDir);`}
</CodeBlock>

### Getting All Environment Variables

<CodeBlock language="javascript">
  {`// Get all environment variables as an object
var allEnv = system.getEnv();
console.log("Total environment variables: " + Object.keys(allEnv).length);

// Iterate through all environment variables
for (var key in allEnv) {
    if (allEnv.hasOwnProperty(key)) {
        console.log(key + " = " + allEnv[key]);
    }
}`}
</CodeBlock>