...
<Method
  signature="system.readRegistry(path, valueName)"
...
>
  ...
  <ReturnValue type="string | number | null">
    Returns the value data if successful. Supports `REG_SZ`, `REG_EXPAND_SZ`, and `REG_DWORD`. Returns `null` if the key or value does not exist.
  </ReturnValue>
</Method>
...
<Method
  signature="system.writeRegistry(path, valueName, data)"
